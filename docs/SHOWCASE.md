...
**How that fix was arrived at is documented too** — including the approach we shipped first and had to reverse, why we rejected porting UFTP's congestion control after reading its source, and the measurement we took two rounds before we understood what it was telling us. See **[METHODOLOGY.md](METHODOLOGY.md)** for the reasoning and [`benchmarks/throughput-runs.md`](benchmarks/throughput-runs.md) for every raw run behind it. Throughput work here is still open: the numbers above are averages, and average throughput turns out to hide a bursty delivery pattern we are actively investigating.
