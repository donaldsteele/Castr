...
- **The "position/identity binding" claim above had a real hole until M4, now closed** — see [[wire-protocol]]'s Manifest section and [[m4-mobile-summary]] for full detail. `MerkleProof.Verify` recomputed the root from the proof's `Steps` but never checked those `Steps` actually committed to the wire-supplied `LeafIndex`, so a relaying peer could keep a genuine chunk's valid proof while relabeling its claimed position, stalling a receiver. Fixed by deriving the committed position from `Steps` inside `Verify` itself. This was a defect in the original M1 primitive that neither M1's nor M3's security test passes caught, since both tested `MerkleProof.Verify` and the position-check guards in isolation rather than exercising a position-relabeling attack through a real session state machine end to end — M4's QA pass closed that methodological gap with new full-session regression tests (see [[m4-mobile-summary]]).
...
- **M4 extended this to the mobile swarm-pull tier** (see [[wire-protocol]]'s new section) with the same authorization boundary, enforced even more strictly by construction: only a sender (`SenderSession.CreateSwarmContentSource()`) can grant the content key to an incoming TCP pull. A receiver serving chunks it already has (`ReceiverSession.CreateSwarmContentSource()`) cannot grant the key even if its code tried to — it never holds the sender's X25519 private key, so this is a cryptographic impossibility, not a policy check that could be bypassed by a bug elsewhere.
