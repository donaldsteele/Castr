...
**Chunks** (256 KB–1 MB) are the hash/repair granularity. Each chunk is split into **wire packets** (**1472 bytes** since M9 — the largest UDP payload that does not IP-fragment at a standard 1500-byte Ethernet MTU; 1200 before that; configurable via `--datagram-size`) for actual UDP datagrams, keeping the repair layer's chunk-index granularity (see [[repair-protocol]]) distinct from the transport layer's datagram granularity.

**Packet 0 is short and every later packet is full.** The Merkle proof rides only on packet 0, so only packet 0 reserves room for it: packet 0 carries `budget − 43 − proofBytes` and every later packet carries `budget − 43`. Until M9 *every* packet was sized against packet 0's proof-carrying envelope, which wasted the proof's bytes on every packet after the first — 307 bytes of every 893-byte datagram at a 100 MiB transfer. See [[m9-datagram-efficiency]] for the measured effect and for why this changes no field layout.

**Implemented in M3** — see [[m3-test-ci-hardening-summary]]. `ChunkPacketizer`/`ChunkPacketAssembler` slice a chunk's already-encrypted ciphertext into identity-keyed packets (`(fileIndex, chunkIndex, fragmentOffset)`, message tag 11); the Merkle inclusion proof rides only on the packet at offset 0. **Keyed by byte offset since M11** — it was `(fileIndex, chunkIndex, packetIndex)` plus a `packetCount` through M9, and a packet index only means something relative to the slicing that produced it, so peers on different `--datagram-size` budgets described mutually unintelligible layouts and could not combine their packets at all. Byte ranges are a property of the ciphertext, so any mix of slicings reassembles; the assembler tracks byte coverage rather than packet arrival, and overlap is resolved first-writer-wins. That change moved the field layout of an existing message, so **`MessageCodec.FormatVersion` went 1 → 2** (bodies are not self-describing; the version byte turns a cross-version peer into a clean rejection at `Decode` rather than a silent misparse) and the fixed envelope shrank 43 → 39 bytes. See [[m11-backlog-clearance]]. A separate, generic `WirePacketizer`/`PacketReassembler` (tag 10) handles fragmentation for oversized control messages (e.g. a large multi-file `MANIFEST`). Splitting happens *after* encryption and hashing, so packetization has zero interaction with the crypto/Merkle layers described below and in [[security-model]] — verified directly by tampering a single wire packet mid-transfer and confirming the existing Merkle-proof/AEAD-tag checks still catch it post-reassembly. A receiver accumulates a chunk's packets across repair rounds and across sources (the same deterministic slicing means a re-sent packet from the sender or a relaying peer is byte-identical to the original), which is what lets large chunks survive real packet loss rather than needing a single lossless round — proven with a 256 KB chunk completing byte-identically at 10% real per-packet UDP loss. At the 1200-byte default of the time, even the 8 KB chunk size every M2 UI surface defaulted to already packetized (confirmed via traffic capture: largest observed datagram exactly 1200 bytes, well under Ethernet's 1500-byte MTU) — the crash/hang bug M2 found and mitigated with a Cli-side `--chunk-size` guard is gone, and that guard has been removed in favor of a generous 16 MiB memory-safety ceiling on reassembly buffering. The default is **1472** since M9; the same capture at that budget shows a largest datagram of exactly 1472 bytes, still unfragmented at a 1500-byte MTU.
