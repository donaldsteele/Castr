...
- `decision` — an architecture/technology decision made for Castr (ADR-style), with rationale.
- `spike-result` — findings from a time-boxed research spike (e.g. library viability checks).
- `protocol` — pages about the wire protocol, repair algorithm, or message formats.
- `security` — trust model, signing, path-safety, and other security-relevant pages.
- `platform-quirk` — OS/runtime-specific behavior (Windows/Linux/macOS/iOS/Android) that shaped a design choice.
- `milestone` — status/summary pages tied to a specific milestone (M0–M5).
