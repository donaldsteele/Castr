...
| M1.5 — Payload encryption retrofit | X25519 keypairs, `JOIN_REQUEST`/`KEY_GRANT` handshake, ChaCha20-Poly1305 chunk encryption, Merkle-over-ciphertext, updated `SenderSession`/`ReceiverSession`, new tests | **Not started — inserted ahead of M2.** Design decided and validated in [[adr-0003-payload-encryption]] (reverses the original M0 "no encryption" call — see [[security-model]]): plaintext chunk payloads leak to any device that can passively observe LAN multicast, not just devices the receiver has chosen to trust. **M1's shipped code still sends plaintext** — this milestone retrofits `Castr.Core`'s wire format before M2 builds CLI/TUI/GUI on top of it, since building those against the current plaintext format would mean redoing that work once encryption lands. Must land, be fully tested (unit: key agreement/wrap-unwrap/encrypt-decrypt/tamper+wrong-AAD rejection; integration: an encrypted-transfer analogue of `EndToEndTransferTests` and `RealTransferRepairTests`), and pass a QA review before M2 starts, per the same definition-of-done every other milestone uses. |
| M2 — CLI, TUI, Desktop GUI (parallel) | `Castr.Cli`, `Castr.Tui`, `Castr.Gui.Desktop`, each with its own test suite | **Blocked on M1.5.** Do not start until payload encryption is retrofitted — CLI/TUI/GUI work built against the current plaintext wire format would need rework once M1.5 changes it. |
...
- **Payload encryption retrofit (M1.5) must land before M2** — see [[adr-0003-payload-encryption]]. The decision was made and validated *after* M1 shipped plaintext, so M1's wire format is currently inconsistent with the confirmed security model. Do not start M2 CLI/TUI/GUI work against the current wire format.
- **Manifest metadata (file names/sizes) remains unencrypted** even after M1.5 lands — [[adr-0003-payload-encryption]] only encrypts chunk payloads by design. Revisit if a deployment needs filename/metadata confidentiality too; documented as a known, deliberate scope boundary, not an oversight.
